...
func (p *MetricsHandler) Start() error {
	...
	if p.listen != "" {
		addr, err := net.ResolveTCPAddr("tcp", p.listen)
		if err != nil {
			return err
		}

		TCPlistener, err := internet.ListenSystem(context.Background(), addr, nil)
		if err != nil {
			if !strings.Contains(err.Error(), "address already in use") {
				return err
			}

			errors.LogWarningInner(context.Background(), err, "metrics listener is already in use; continue without direct metrics listener")
		} else {
